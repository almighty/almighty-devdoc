...
Different services will want different fields with different semantics in their work items. For example, both awaterfall and a scrum planning will have a "task" item that includes fields like "owner", "severity", "description", etc. However, they will also have fields that differ, like "target release" or "sprint". 
